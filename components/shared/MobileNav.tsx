      ...
      <nav className="flex gap-2">
        <SignedIn>
          ...
          <Sheet>
            ...
            <SheetContent className="sheet-contetnt sm:w-64">
              <>
                <Image src="/assets/images/logo-text.svg" alt="logo" height={23} width={152} className="" />
                <ul className="header-nav_elements">
                  {navLinks.slice(0, 6).map((link) => {
                    const isActive = link.route === pathname;
                    return (
                      <li key={link.route} className={`${isActive && "gradient-text"} p-18 flex whitespace-nowrap text-dark-700`}>
                        <Link className="sidebar-link" href={link.route} aria-current={isActive ? "page" : "false"}>
